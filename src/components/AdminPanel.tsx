...
export const AdminPanel: React.FC = () => {
  const { authenticProducts, scans, addProduct, removeProduct, fetchProducts, fetchScans, isLoading, error } = useAuthStore();
      ...
      <div className="max-w-4xl mx-auto p-4">
        <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage scans</p>
            </div>
        </div>
        <div className="grid gap-4">
          {scans.map((scanLog) => (
            <div>
              <p>{scanLog.id} IP Address: {scanLog.ipAddress} Timestamp: {scanLog.timestamp} QR Code: {scanLog.qrCode}</p>
            </div>
          ))}
        </div>
